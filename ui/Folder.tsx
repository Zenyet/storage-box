...
export default function Folder({ locked, folderName, href_ }: {
  ...
  return (
    <Link href={href} onClick={e => e.preventDefault()}
      ...
      <p
        className='max-w-[100%] overflow-hidden text-ellipsis text-gray-700 text-[13px] mt-0.5 px-1 rounded-[4px] group-focus:bg-folder-n-hv group-focus:text-white'>{folderName}</p>
    </Link>
  );
}